...
class ModelExtensionTotalVolumeDiscount extends Model {
	public function getTotal($total) {
		if ($this->config->get('module_discounts_pack_status') && $this->cart->hasProducts() && $this->config->get('total_volume_discount_status')) {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				$volume_discount = $this->model_extension_module_discount->getVolumeDiscount($product['product_id']);
				...
				if ($volume_discount) {
					...
					if (empty($discount_data[strtolower($volume_discount['name'])])) {
						...
						$discount_data[strtolower($volume_discount['name'])] = array(
							...
							'value'      => -$discount,
							'sort_order' => $this->config->get('total_volume_discount_sort_order')
						);	
					} else {
