...
class ModelExtensionTotalCustomerGroupDiscount extends Model {
	public function getTotal($total) {
		if ($this->config->get('module_discounts_pack_status') && $this->cart->hasProducts() && $this->config->get('total_customer_group_discount_status')) {
			...
			$customer_group_discount = $this->model_extension_module_discount->getCustomerGroupDiscount($customer_group_id);
			...
			if ($customer_group_discount) {
				...
				foreach ($this->cart->getProducts() as $product) {
					...
					if ($this->config->get('module_discounts_pack_override_special_price') == 'exclusive' && $this->hasSpecialPrice($product['product_id'], $customer_group_id)) {
						$discount = 0;
					} else {
