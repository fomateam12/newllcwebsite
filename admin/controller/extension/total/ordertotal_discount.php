<?php
class ControllerExtensionTotalOrdertotalDiscount extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('total_ordertotal_discount', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/ordertotal_discount', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/total/ordertotal_discount', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/total/ordertotal_discount', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/total/ordertotal_discount')) {
	...
	public function install() {
		...
		$sql = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX ."ordertotal_discount` (`ordertotal_discount_id` int(11) NOT NULL AUTO_INCREMENT, `ordertotal` int(11) NOT NULL, `customer_group_id` int(11) NOT NULL, `priority` int(5) NOT NULL DEFAULT '1', `type`char(1) NOT NULL, ";
  		$sql .= "`percentage` decimal(15,4) NOT NULL DEFAULT '0.0000', `date_start` date NOT NULL DEFAULT '0000-00-00', `date_end` date NOT NULL DEFAULT '0000-00-00', PRIMARY KEY (`ordertotal_discount_id`), KEY `ordertotal` (`ordertotal`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;";
	...
	public function uninstall() {
		...
		$this->db->query("DROP TABLE `" . DB_PREFIX ."ordertotal_discount`");
		
		$this->db->query("DELETE FROM `". DB_PREFIX ."extension` WHERE `code` = 'ordertotal_discount';");
		$this->db->query("DELETE FROM `". DB_PREFIX ."setting` WHERE `code` = 'total_ordertotal_discount';");
