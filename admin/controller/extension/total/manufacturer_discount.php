<?php
class ControllerExtensionTotalManufacturerDiscount extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('total_manufacturer_discount', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/manufacturer_discount', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/total/manufacturer_discount', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/total/manufacturer_discount', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/total/manufacturer_discount')) {
	...
	public function install() {
		...
		$sql = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX ."manufacturer_discount` (`manufacturer_discount_id` int(11) NOT NULL AUTO_INCREMENT, `manufacturer_id` int(11) NOT NULL, `customer_group_id` int(11) NOT NULL, `priority` int(5) NOT NULL DEFAULT '1', ";
  		$sql .= "`percentage` decimal(15,4) NOT NULL DEFAULT '0.0000', `date_start` date NOT NULL DEFAULT '0000-00-00', `date_end` date NOT NULL DEFAULT '0000-00-00', PRIMARY KEY (`manufacturer_discount_id`), KEY `manufacturer_id` (`manufacturer_id`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;";
	...
	public function uninstall() {
		...
		$this->db->query("DROP TABLE `" . DB_PREFIX ."manufacturer_discount`");
		
		$this->db->query("DELETE FROM `". DB_PREFIX ."extension` WHERE `code` = 'manufacturer_discount';");
		$this->db->query("DELETE FROM `". DB_PREFIX ."setting` WHERE `code` = 'total_manufacturer_discount';");
