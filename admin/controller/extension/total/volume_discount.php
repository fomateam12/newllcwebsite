...
class ControllerExtensionTotalVolumeDiscount extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('total_volume_discount', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/volume_discount', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/total/volume_discount', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/total/volume_discount', $data));
	}
	...
	public function install() {
		...
		$this->db->query("INSERT INTO `". DB_PREFIX ."setting` (`setting_id`, `store_id`, `code`, `key`, `value`, `serialized`) VALUES (NULL, '0', 'total_volume_discount', 'total_volume_discount_sort_order', '6', '0'); ");
	...
	public function uninstall() {
		...
		$this->db->query("DELETE FROM `". DB_PREFIX ."setting` WHERE `code` = 'total_volume_discount';");
