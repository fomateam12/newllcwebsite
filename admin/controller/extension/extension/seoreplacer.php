...
class ControllerExtensionExtensionSEOReplacer extends Controller {
	private $error = array(); 
	
	public function index() {
		$this->load->language('extension/extension/seoreplacer');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($seoreplacer['customtitles']) && ($seoreplacer['customtitles'])) {
				...
				$this->db->query("UPDATE " . DB_PREFIX . "manufacturer_description SET custom_title = REPLACE (custom_title, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");
				}
				
			if (isset($seoreplacer['seourls']) && ($seoreplacer['seourls'])) {			
				$this->db->query("UPDATE " . DB_PREFIX . "seo_url SET keyword = REPLACE (keyword, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE store_id = 0 and language_id = '".$seoreplacer['language_id']."';");			
				}
				
			if (isset($seoreplacer['customalts']) && ($seoreplacer['customalts'])) {			
				$this->db->query("UPDATE " . DB_PREFIX . "product_description_seo SET custom_alt = REPLACE (custom_alt, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");			
				}
				
			if (isset($seoreplacer['customalts']) && ($seoreplacer['customh1tags'])) {			
				$this->db->query("UPDATE " . DB_PREFIX . "product_description_seo SET custom_h1 = REPLACE (custom_h1, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");			
				}
				
			if (isset($seoreplacer['customalts']) && ($seoreplacer['customh2tags'])) {			
				$this->db->query("UPDATE " . DB_PREFIX . "product_description_seo SET custom_h2 = REPLACE (custom_h2, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");			
				}
				
			if (isset($seoreplacer['customalts']) && ($seoreplacer['customimagetitles'])) {			
				$this->db->query("UPDATE " . DB_PREFIX . "product_description_seo SET custom_imgtitle = REPLACE (custom_imgtitle, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");			
				}
			...
			$this->response->redirect($this->url->link('extension/extension/seoreplacer', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
		...
		$data['cancel'] = $this->url->link('extension/extension/seoreplacer', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
       		'text'      => 'Auto Links',
			'href'      => $this->url->link('extension/extension/seoreplacer', 'user_token=' . $this->session->data['user_token'], 'SSL'),       		
      		'separator' => ' :: '
   		);
		
   					
		$data['action'] = $this->url->link('extension/extension/seoreplacer', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/extension/seoreplacer', $data));
	} 
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/extension/seoreplacer')) {
