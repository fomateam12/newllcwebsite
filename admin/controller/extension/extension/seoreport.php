...
class ControllerExtensionExtensionSeoReport extends Controller {
	private $error = array(); 
     
  	public function index() {
		$this->load->language('extension/extension/seoreport');
		...
		$this->load->model('extension/extension/seoreport');
  	...
  	private function getList() {				
		...
		if (isset($this->request->get['page'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
		...
		$product_total = $this->model_extension_extension_seoreport->getTotalSEOs($fdata);
			
		$results = $this->model_extension_extension_seoreport->getSEOs($fdata);
		...
		$data['action'] = $this->url->link('extension/extension/seoreport/changeLanguage&user_token=', 'user_token=' . $this->session->data['user_token'].'&lang=', 'SSL');
		...
		foreach ($data['languages'] as $language) {
			if ($language['language_id'] == $this->session->data['language_id']) {$data['selected_language'] = $language['name'];}
		}
		...
		$data['sort_name'] = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, 'SSL');
		$data['sort_type'] = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . '&sort=type' . $url, 'SSL');
		$data['sort_keyword'] = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . '&sort=keyword' . $url, 'SSL');
		$data['sort_meta_keyword'] = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . '&sort=meta_keyword' . $url, 'SSL');
		$data['sort_meta_description'] = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . '&sort=meta_description' . $url, 'SSL');
		$data['sort_tags'] = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . '&sort=tags' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/extension/seoreport', $data));
  	}
	...
	public function changeLanguage()
    {
        ...
        $this->response->redirect($this->url->link('extension/extension/seoreport', 'user_token=' . $this->session->data['user_token'], 'SSL'));
    }
}
?>