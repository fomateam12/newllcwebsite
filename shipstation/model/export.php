...
class ModelExport extends Model {
    ...
    public function getOrderProducts($order_id) {
        $query = $this->db->query("SELECT *, op.quantity AS quantity, op.price AS price FROM " . DB_PREFIX . "order_product op INNER JOIN " . DB_PREFIX . "product p ON op.product_id = p.product_id WHERE p.shipping = 1 AND op.order_id = '" . (int) $order_id . "'");
        return $query->rows;
    }
