...
//var_dump(implode('|', array_keys(self::$map)));
		self::$regex = '/(' . implode('|', array_keys(self::$map)) . ')/u';
    //echo self::$regex;
	...
	public static function downcode ($text, $language = "") {
		...
		if (preg_match_all (self::$regex, $text, $matches)) {
          ...
          //var_dump('/'.$char.'/'.' -- '. self::$map[$char].' -- '. $text);
					$text = preg_replace ('/'.$char.'/', self::$map[$char], $text);
				}
			}
		}
    
    // process 2 times to handle some characters made with various accentuations
    if (preg_match_all (self::$regex, $text, $matches)) {
			for ($i = 0; $i < count ($matches[0]); $i++) {
				$char = $matches[0][$i];
				if (isset (self::$map[$char])) {
	...
	public static function filter ($text, $length = 999, $language = "", $file_name = false, $use_remove_list = true, $lower_case = true, $treat_underscore_as_space = true) {
		...
		$remove_pattern = ($file_name) ? '/[^_\-.\-a-zA-Z0-9\s]/' : '/[^\s_\-a-zA-Z0-9]/';
